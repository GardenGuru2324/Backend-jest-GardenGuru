...
import { objectMessages, objectStatusCodes } from "../../src/lib/getPlants/getPlantsOfPlantLocationObjects.mjs";
...
describe("Get all the plants of a certain location of an user", () => {
  ...
  objectStatusCodes.forEach((obj) => {
    it(`Should ${obj.it}`, async () => {
      const result = await client.getAllPlantsOfPlantLocationOfUser(obj.userId, obj.plantLocationName);
  ...
  objectMessages.forEach((obj) => {
    it(`Should ${obj.it}`, async () => {
      const response = await client.getAllPlantsOfPlantLocationOfUser(obj.userId, obj.plantLocationName);
  ...
  it(`Should return the plants of specific location, of specific user`, async () => {
    const response = await client.getAllPlantsOfPlantLocationOfUser("jest_user_1", "jest_plantLocation_1");
    const result = JSON.parse(response.text);

    const allPlants = await client.getAllPlants();
    const expectedPlants = JSON.parse(allPlants.text).filter((plant) => plant.userId === "jest_user_1" && plant.locationId === "jest_plantLocation_1");
